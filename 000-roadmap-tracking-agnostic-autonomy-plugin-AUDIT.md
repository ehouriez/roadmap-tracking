...
| Format hook Codex (`sessionStart` / `context: inject`) | Mineur | ⚠️ Non vérifié | Pas d'accès à la source/doc Codex dans la session de correction. Le format reste **spéculatif et non confirmé**. Sans impact sur Claude Code (fichier non référencé dans `plugin.json`). À confirmer côté Codex avant tout usage réel du hook Codex. |
